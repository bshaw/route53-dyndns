...
def get_zone(zone_to_update):
    ...
    hosted_zones = zone['HostedZones']
    if not hosted_zones:
        raise ValueError(f'No hosted zone found for {zone_to_update}')
    hosted_zone_id = hosted_zones[0]['Id'].split('/')[2]
...
def get_record_ip(hosted_zone_id, record_to_update):
    ...
    records = record_set['ResourceRecordSets']
    if not records:
        raise ValueError(f'No A record found for {record_to_update}')
    record_ip = records[0]['ResourceRecords'][0]['Value']
...
def update_record(hosted_zone_id, record_to_update, current_ip):
    def get_change_status(change_id):
        client = boto3.client('route53')
        change_status = client.get_change(Id=change_id)
...
def main():
    parser = argparse.ArgumentParser(
        description='Update a Route53 A record with the current external IP address.',
    )
    parser.add_argument(
        '-r', '--record',
        help='specify the DNS A record to update',
    )
    parser.add_argument(
        '-v', '--verbose',
    ...
    if status == 'INSYNC':
        logging.info(
            f'Updated A record {record_to_update} in hosted zone '
            f'{zone_to_update} ({hosted_zone_id}) from {record_ip} to {current_ip}'
        )
